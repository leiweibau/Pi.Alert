...
DNS is resolved before connecting and the connection is pinned to the validated address. The same validation and DNS resolution are repeated for every redirect. Up to ten redirects are followed within a fixed total time limit, and repeated URL/cookie states are reported as redirect loops instead of being confused with a longer valid redirect chain. Known cloud metadata addresses remain blocked. Response bodies are not downloaded by the monitor.

Redirect cookies are held only in memory for the duration of one service check. Standard domain, path, and Secure-cookie rules apply, and cookie count, value size, and the resulting request header are bounded. Cookie values are neither logged nor stored. This allows local services that set a short-lived state cookie before redirecting to their final page to be monitored without creating a persistent browser-like session.
...
For HTTPS services, the certificate subject, issuer, validity start and validity end are captured from the final pinned TLS connection and stored during the initial insert. Self-signed and otherwise privately issued certificates are supported; the monitor records their metadata but does not require a trusted CA chain. The initial SSL change flag is `0`, because no previous certificate exists for comparison. HTTP services use empty SSL fields.

The monitor stores the configured URL, final HTTP status, first status in a redirect chain, redirect count or failure note, total response latency, selected target IP, scan time, and HTTPS certificate metadata. Status, latency, IP, and certificate changes continue to feed the service history, journal, and notification workflow. Redirect diagnostics contain only bounded per-hop status, host/path, and target-IP information; URL query values and cookie values are redacted. Response bodies, form data, and persistent session data are not collected.

Failures are classified as invalid URL, DNS resolution failure, connection failure, TLS failure, network-policy block, redirect loop, or redirect-limit exhaustion. Only a network-policy violation is described as blocked. A valid but currently unreachable service can still be saved with status `0` and will be checked again by the periodic monitor.
