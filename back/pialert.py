...
def create_new_devices():
    ...
    # Fritzbox - Insert events for new devices
    print_log ('New devices - 7 Fritzbox Events')
    sql.execute ("""INSERT INTO Events (eve_MAC, eve_IP, eve_DateTime,
                        eve_EventType, eve_AdditionalInfo,
                        eve_PendingAlertEmail)
                    SELECT FB_MAC, IFNULL (FB_IP,'-'), ?, 'New Device',
                        '(Fritzbox) ' || FB_Vendor, 1
                    FROM Fritzbox_Network
                    WHERE NOT EXISTS (SELECT 1 FROM Devices
                                      WHERE dev_MAC = FB_MAC) """,
                    (startTime, ) ) 

    # Fritzbox - Create New Devices
    print_log ('New devices - 8 Fritzbox Create devices')
    sql.execute ("""INSERT INTO Devices (dev_MAC, dev_name, dev_Vendor,
                        dev_LastIP, dev_FirstConnection, dev_LastConnection,
                        dev_ScanCycle, dev_AlertEvents, dev_AlertDeviceDown,
                        dev_PresentLastScan)
                    SELECT FB_MAC, FB_Name, FB_Vendor, IFNULL (FB_IP,'-'),
                        ?, ?, 1, 1, 0, 1
                    FROM Fritzbox_Network
                    WHERE NOT EXISTS (SELECT 1 FROM Devices
                                      WHERE dev_MAC = FB_MAC) """,
    ...
    # Mikrotik - Insert events for new devices
    print_log ('New devices - 9 Mikrotik Events')
    sql.execute ("""INSERT INTO Events (eve_MAC, eve_IP, eve_DateTime,
                        eve_EventType, eve_AdditionalInfo,
                        eve_PendingAlertEmail)
                    SELECT MT_MAC, IFNULL (MT_IP,'-'), ?, 'New Device',
                        '(Mikrotik) ' || MT_Vendor, 1
                    FROM Mikrotik_Network
                    WHERE NOT EXISTS (SELECT 1 FROM Devices
                                      WHERE dev_MAC = MT_MAC) """,
                    (startTime, ) ) 

    # Mikrotik - Create New Devices
    print_log ('New devices - 10 Mikrotik Create devices')
    sql.execute ("""INSERT INTO Devices (dev_MAC, dev_name, dev_Vendor,
                        dev_LastIP, dev_FirstConnection, dev_LastConnection,
                        dev_ScanCycle, dev_AlertEvents, dev_AlertDeviceDown,
                        dev_PresentLastScan)
                    SELECT MT_MAC, MT_Name, MT_Vendor, IFNULL (MT_IP,'-'),
                        ?, ?, 1, 1, 0, 1
                    FROM Mikrotik_Network
                    WHERE NOT EXISTS (SELECT 1 FROM Devices
                                      WHERE dev_MAC = MT_MAC) """,
    ...
    # Unifi - Insert events for new devices
    print_log ('New devices - 11 Unifi Events')
    sql.execute ("""INSERT INTO Events (eve_MAC, eve_IP, eve_DateTime,
                        eve_EventType, eve_AdditionalInfo,
                        eve_PendingAlertEmail)
                    SELECT UF_MAC, IFNULL (UF_IP,'-'), ?, 'New Device',
                        '(Unifi) ' || UF_Vendor, 1
                    FROM Unifi_Network
                    WHERE NOT EXISTS (SELECT 1 FROM Devices
                                      WHERE dev_MAC = UF_MAC) """,
                    (startTime, ) ) 

    # Unifi - Create New Devices
    print_log ('New devices - 12 Unifi Create devices')
    sql.execute ("""INSERT INTO Devices (dev_MAC, dev_name, dev_Vendor,
                        dev_LastIP, dev_FirstConnection, dev_LastConnection,
                        dev_ScanCycle, dev_AlertEvents, dev_AlertDeviceDown,
                        dev_PresentLastScan)
                    SELECT UF_MAC, UF_Name, UF_Vendor, IFNULL (UF_IP,'-'),
                        ?, ?, 1, 1, 0, 1
                    FROM Unifi_Network
                    WHERE NOT EXISTS (SELECT 1 FROM Devices
                                      WHERE dev_MAC = UF_MAC) """,
