...
reset_permissions() {
  ...
  sudo chgrp -R www-data $PIALERT_HOME/db
  sudo chmod -R 775 $PIALERT_HOME/db
  sudo chgrp -R www-data $PIALERT_HOME/config
  sudo chmod -R 775 $PIALERT_HOME/config
}
...
create_backup() {
  ...
  cd "$INSTALL_DIR"
  tar cvf "$INSTALL_DIR"/pialert_update_backup_`date +"%Y-%m-%d_%H-%M"`.tar pialert --checkpoint=100 --checkpoint-action="ttyout=."
  echo ""
}
